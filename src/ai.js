...
export async function writeArticle(topic, settings, config, options = {}) {
  ...
  const rules = Array.isArray(settings.evolutionRules) ? settings.evolutionRules.filter(Boolean) : [];
  const evolutionPrompt = rules.length > 0
    ? `\n\n【必须严格遵循的已复盘 Agent 进化规则】：\n${rules.map((r, i) => `${i + 1}. ${r}`).join("\n")}\n`
    : "";
  const referencePrompt = refArticle
    ? `\n\n【风格范文仿写要求】：\n请深度分析并模仿以下参考范文的叙写套路、开篇 hook 钩子、小节推进逻辑、段落节奏与金句总结习惯，但保持新文章在选题上的独立论据与具体事实：\n---参考范文开始---\n${refArticle.slice(0, 3000)}\n---参考范文结束---\n`
    : "";
  const sectionHint =
