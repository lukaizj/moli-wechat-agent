...
- 🎨 **Antigravity / Gemini 生图引擎**：支持在 **GPT** (OpenAI / Codex) 与 **Gemini** (Antigravity 生图) 之间动态切换生图引擎，可在 Web 界面与栏目中一键配置。
- 📸 **自定义图片上传与 Agent 自动排版**：支持上传本地图片，Agent 自动将第 1 张图片设为主封面，后续图片按顺序嵌入正文各个小节卡片中并推送至微信草稿箱。
...
## 🎨 Antigravity / Gemini 生图引擎配置与切换

**墨流** 支持在 **GPT (ChatGPT 会员 / OpenAI)** 与 **Gemini (Antigravity 生图 CLI)** 之间无缝切换：

1. **界面选择切换**：在 Web 界面「公众号设定」或「栏目管理」中的 **生图引擎** 下拉框，可直接选择 `GPT` 或 `Gemini (Antigravity 生图)`；
2. **配置文件（.env）**：
   ```dotenv
   # 使用 Gemini (Antigravity) 本地 CLI 生图
   IMAGE_PROVIDER=gemini
   ANTIGRAVITY_PATH=gemini
   ```
...
你可以上传本地图片，由 Agent 自动完成文章封面与小节插图的内联排版：

1. **上传图片**：在控制台 **「✦ 模仿推文生成」** 弹窗中点击 **「选择图片文件」** 选择本地图片；
2. **Agent 自动排版逻辑**：
   - **主封面图**：上传的第 1 张图片自动设为主封面；
   - **正文小节插图**：后续图片按顺序自动分配嵌入文章各个小节卡片中；
   - **AI 智能补全**：若上传图片少于文章小节数，剩余小节会自动由选定的 AI 引擎（GPT 或 Gemini）补全配图；
3. **推送微信草稿箱**：自动处理本地图片上传至微信素材库并嵌入原生 HTML 排版，直接写入草稿箱。
