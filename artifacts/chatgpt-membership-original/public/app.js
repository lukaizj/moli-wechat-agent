...
function renderIntegrations() {
  const { openai, wechat } = state.integrations;
  $("#openai-dot").classList.toggle("ready", openai);
  $("#wechat-dot").classList.toggle("ready", wechat);
  $("#openai-label").textContent = openai ? "已接入" : "演示模式";
  $("#wechat-label").textContent = wechat ? "已接入" : "待配置";
  $("#settings-openai").textContent = `${openai ? "●" : "○"} 内容引擎${openai ? "已接入" : "演示模式"}`;
  $("#settings-wechat").textContent = `${wechat ? "●" : "○"} 微信公众号${wechat ? "已接入" : "待配置"}`;
  $("#settings-openai").classList.toggle("ready", openai);
...
async function startRun() {
  try {
    ...
    toast(state.integrations.openai ? "已开始研究选题，完成后会自动进入草稿箱" : "已启动演示流程，接入密钥后会生成正式稿");
    await loadState();
  } catch (error) {
    toast(error.message, true);
  }
